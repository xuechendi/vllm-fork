...
@ray.remote(num_gpus=0, max_calls=1, resources={'HPU': 1})
def all_reduce_test_worker(tensor_parallel_size: int, rank: int,
    ...
    class DummyModel(torch.nn.Module):
        ...
        def forward(self, x):
            ...
            ret = torch.distributed.all_reduce(x, group=self.group)
            return ret
    # it is important to delete the CUDA_VISIBLE_DEVICES environment variable
    # so that each worker can see all the GPUs
    # they will be able to set the device to the correct GPU
    del os.environ["CUDA_VISIBLE_DEVICES"]
    device = torch.device(f"hpu")
    torch.hpu.set_device(device)
    init_test_distributed_environment(1, tensor_parallel_size, rank,
                                      distributed_init_port)
    num_elements = 8
    input_ = torch.arange(num_elements, dtype=torch.int32, device="hpu")
    embedding_weights = torch.randn(8, 8, dtype=torch.float32)
    #weights1 = torch.randn(8, 8, dtype=torch.float32)
    if rank == 0:
        broadcast_tensor_dict({"data":embedding_weights}, src=0)
    else:
        embedding_weights_dict = broadcast_tensor_dict(src=0)
        embedding_weights = embedding_weights_dict["data"]
        
    model = DummyModel(embedding_weights)
    #model = AllReduceModel()
    model = htorch.hpu.wrap_in_hpu_graph(
        model, disable_tensor_cache=True)
    
    out = model(input_)
    
    print(f"rank is {rank}, after allreduce is {out}")
    sleep(5)
...
@ray.remote(num_gpus=1, max_calls=1, resources={'HPU': 1})
