...
num_prompts=224
request_rate=8
log_name="[0306-staticquant]online-gaudi3-${gpu_utils}util-TPparallel${tp_parrallel}-${moe_n_slice}moegroups_nprompt${num_prompts}_rrate${request_rate}_bs${bs}_i${in_len}_o${out_len}_mdllen${total_len}"
...
python -m vllm.entrypoints.openai.api_server \
    ...
    --trust_remote_code 2>&1 | tee benchmark_logs_upstream/${log_name}_serving.log &
pid=$(($!-1))

until [[ "$n" -ge 1000 ]] || [[ $ready == true ]]; do
    n=$((n+1))
    if grep -q "Started server process" benchmark_logs_upstream/${log_name}_serving.log; then
        break
    fi
    sleep 5s
done
...
hl-smi -l > tee benchmark_logs_upstream/${log_name}_smi.log &
hl_pid=$(($!-1))


start_time=$(date +%s)
echo "Start to benchmark"
python benchmarks/benchmark_serving.py --backend vllm --model ${model} --tokenizer ${tokenizer} --dataset-name sonnet --dataset-path benchmarks/sonnet.txt --request-rate ${request_rate} --num-prompts ${num_prompts} --port 18080 --sonnet-input-len ${in_len} --sonnet-output-len ${out_len} --sonnet-prefix-len 100 2>&1 | tee benchmark_logs_upstream/${log_name}_run1.log
end_time=$(date +%s)
